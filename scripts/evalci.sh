...
BIN="$WORK/q4tab-evalci"
MODEL="$WORK/q4tab-evalci-model.bin"
REPORT="$WORK/q4tab-evalci-report.json"
BASELINE=testdata/eval-baseline.json

go build -o "$BIN" ./cmd/q4tab

# Train on the engine's own source. The holdout lives in testdata/ so
# it is never part of the training set.
Q4TAB_MODEL="$MODEL" "$BIN" index -root cmd -root internal -o "$MODEL"

if [ "$1" = "--write-baseline" ]; then
    Q4TAB_MODEL="$MODEL" "$BIN" eval -root testdata/holdout \
